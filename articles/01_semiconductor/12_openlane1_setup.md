...
OpenLane は、  
**RTL から GDSII までを一気通貫で流せるオープンな ASIC 設計フロー**です。  
...
そのための **OpenLane1（Docker ベース）環境構築**を整理します。
...
- **OpenLane1**：Docker ベースの従来フロー  
- OpenLane2：Python ベース（Nix 推奨）の新系統  

本記事は **OpenLane1 のみ**を扱います。
...
フローを最後まで流し、  
`runs/` 以下に **GDS が生成されていること**を確認します。
